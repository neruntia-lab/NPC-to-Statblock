...
function buildStatblockHtml(actor, portrait) {
  ...
  return `<div style="font-family:${bodyFont}, sans-serif;background-color:#fdf5e6;color:${textColor};padding:20px;margin:auto;max-width:780px;border:2px solid #333;box-shadow:5px 5px 15px rgba(0,0,0,0.3);display:flow-root;">
    <style>
      h1,h2{font-family:${titleFont}, serif;color:${titleColor};text-align:center;font-variant:small-caps;}
      ...
      img.portrait{float:${imageSide};width:150px;margin:${imageSide==='left'?'0 10px 10px 0':'0 0 10px 10px'};}
    </style>
    <h1>${actor.name}</h1>
    <div class="statblock-header">${header}</div>
    ...
    <div class="statblock-section traits">
      ${sections.traits.length ? '<h2>Traits</h2>' : ''}
      ${buildItems(sections.traits)}
    </div>
    <div class="statblock-section actions">
      ${sections.actions.length ? '<h2>Actions</h2>' : ''}
      ${buildItems(sections.actions)}
    </div>
